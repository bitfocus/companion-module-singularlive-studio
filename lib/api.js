...
class SingularLive {
            ...
            fetch(url + this.token + '/model', this.GETOption())
                ...
                .then(result => {
                    ...
                    for (let i = 0; i < data.length; i++) {
                        if (data[i].controlnode)
                            elements.push({
                                id: data[i].id,
                                name: data[i].name,
                                nodes: Object.entries(data[i].controlnode.model.fields).map((entry) => {
                                    return {
                                        [entry[1].title]: entry[1].type
                                    }
                                })
                            })
                    }
    ...
    updateTimer(controlnode, value) {
        if (controlnode === "Select timer control" || value === 'Select action')
            return
